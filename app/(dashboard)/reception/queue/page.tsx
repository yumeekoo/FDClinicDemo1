...
export default function DedicatedQueuePage() {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-950 text-white">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-400 font-semibold">Đang tải bảng hàng đợi phòng khám...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-105 p-6 space-y-6">
      {/* Top Banner / TV Header */}
      <header className="flex justify-between items-center border-b border-slate-800 pb-4">
        <div className="flex items-center space-x-4">
          <div className="h-12 w-12 rounded-2xl bg-gradient-to-tr from-blue-600 to-emerald-500 flex items-center justify-center text-white font-extrabold text-2xl">
            CH
          </div>
          <div>
            <h1 className="text-2xl font-black text-white uppercase tracking-wider">Clinic Hub Queue Screen</h1>
            <p className="text-slate-400 text-sm font-semibold">{activeBranchName || "Đang tải chi nhánh..."}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 flex-1">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-emerald-500 bg-slate-900/40 backdrop-blur-xl h-full flex flex-col">
            <CardHeader className="border-b border-slate-800 bg-emerald-500/10 py-4">
            ...
            <CardContent className="p-6 space-y-4 flex-1 overflow-y-auto">
              {callingQueue.length === 0 ? (
                ...
                callingQueue.map((item) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-xl font-black text-white uppercase">{item.patient.fullName}</span>
        ...
        <div className="lg:col-span-3">
          <Card className="border-slate-800 bg-slate-900/40 backdrop-blur-xl h-full flex flex-col">
            <CardHeader className="border-b border-slate-800 bg-slate-950/20 py-4">
              <div className="flex justify-between items-center">
                <CardTitle className="text-lg font-black text-slate-300 uppercase tracking-widest">
              ...
              {waitingQueue.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {waitingQueue.map((item, index) => (
                    <div
                      key={item.visit.id}
                      className="p-4 rounded-xl border border-slate-800 bg-slate-950/30 flex items-center justify-between"
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-extrabold text-white text-base">{item.patient.fullName}</span>
                        </div>
                        <p className="text-[10px] text-slate-500 font-mono ml-8">
      ...
      {/* Footer Banner */}
      <footer className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl flex justify-between items-center text-xs text-slate-400 font-semibold">
